...
type NodePoolRecommender interface {
	RecommendNodePools(attr string, req recommender.ClusterRecommendationReq, layout []recommender.NodePool, odVms []recommender.VirtualMachine, spotVms []recommender.VirtualMachine) []recommender.NodePool
}
...
func (s *nodePoolSelector) RecommendNodePools(attr string, req recommender.ClusterRecommendationReq, layout []recommender.NodePool, odVms []recommender.VirtualMachine, spotVms []recommender.VirtualMachine) []recommender.NodePool {
	...
	if len(odVms) > 0 {
		if req.OnDemandPct == 0 && !req.AllowDiversify {
	...
	if !req.AllowDiversify {
		if sumSpotValue > 0 {
			// find cheapest spot instance from the list - based on price per attribute
			selectedSpot := spotVms[0]
			for _, vm := range spotVms {
				if vm.AvgPrice/vm.GetAttrValue(attr) < selectedSpot.OnDemandPrice/selectedSpot.GetAttrValue(attr) {
					selectedSpot = vm
				}
			}
			spotNodesToAdd := int(math.Ceil(sumSpotValue / selectedSpot.GetAttrValue(attr)))

			if layout == nil {
				spotNps = append(spotNps, recommender.NodePool{
					SumNodes: spotNodesToAdd,
					VmClass:  recommender.Spot,
					VmType:   selectedSpot,
				})
			} else {
				for i, np := range spotNps {
					if np.VmType.Type == selectedSpot.Type {
						spotNps[i].SumNodes += spotNodesToAdd
					}
				}
			}
		}
